...
void process_frame(void)
{
	...
	noiseThreshold1();	/* Performs thresholding based on max{LAMBDA, 1-|N|/|X|}*/
...
complex minOfThree (complex in1, complex in2, complex in3) {
	...
	return rmul(ATTEN,complexLocal);
}
...
void complexToFloatDelayed (void) {
	...
	for (idxFreq = 0; idxFreq < FFTLEN; idxFreq++ ){
		...
		if(ratio > musicalThreshold && outDelay1X[idxFreq] > killThreshold ){			
