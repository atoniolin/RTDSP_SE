...
#define enhLFP 1
#define enhLFPNoise 1
//#define enhLPFPOWER 1
#define KSCALE 0.0821/*for TAU 80ms*/
...
void process_frame(void)
{
	...
	#ifdef enhLFP
	//perform LPF Filtering
		#ifdef enhLPFPOWER
		ALPHA = 2;
		for (idxFreq = 0 ; idxFreq < FFTLEN ; idxFreq ++){
			power = thisMag[idxFreq]*thisMag[idxFreq];
			thisLPFMag[idxFreq] = sqrt((1-KSCALE)*power+KSCALE*(thisLPFMag[idxFreq]));
		}
		#else
		for (idxFreq = 0 ; idxFreq < FFTLEN ; idxFreq ++){
		...
		#endif
	//make M1 noise estimate based on the LPF version.
	for (idxFreq = 0 ; idxFreq < FFTLEN ; idxFreq ++){
	...
	for (idxFreq = 0 ; idxFreq < FFTLEN ; idxFreq ++){
		...
		//outframe[idxFreq] = cplxBuf[idxFreq].r;
	}
	/********************************************************************************/
